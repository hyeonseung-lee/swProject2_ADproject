...
class DetectTranslator():
    ...
    def __init__(self, targetLang):
        ...
        self.trans_url = "https://openapi.naver.com/v1/papago/n2mt"
        self.detect_url = "https://openapi.naver.com/v1/papago/detectLangs"

        self.target_langCode = {"한국어" : "ko",
                           "영어" : "en",
                           "중국어 간체" : "zh-CN",
                           "중국어 번체" : "zh-TW",
                           "스페인어" : "es",
    ...
    def translateText(self, text):
        ...
        # request to WEB
        detect_request = urllib.request.Request(self.detect_url)
        detect_request.add_header("X-Naver-Client-Id", self.client_id)
        detect_request.add_header("X-Naver-Client-Secret", self.client_secret)
        ...
        # request to WEB
        trans_request = urllib.request.Request(self.trans_url)
        trans_request.add_header("X-Naver-Client-Id",self.client_id)
        trans_request.add_header("X-Naver-Client-Secret",self.client_secret)
